...
Changes can be rolled back to the last commit with [`Tx::rollback`](./struct.Tx.html#method.rollback)

A trait, [`Tx::AsTx`](./trait.AsTx.html) can be imported to implement `Self::tx(&mut self) -> Tx<Self>` on `&mut T`
